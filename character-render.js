...
function characterPose(entity,previous=0) {
    if(entity.motion){const {dx,dy,distance}=entity.motion;const direction=(Math.round(Math.atan2(dy,dx)/(Math.PI/4))+8)%8;
        return {direction,frame:entity.walking?1+Math.floor(distance/WALKING_CONFIG.stridePixels*6)%6:0};}
...
function createRasterCharacter(scene,entity) {
    const pose=characterPose(entity),modern=scene.textures.exists('people-v2')&&scene.textures.get('people-v2').has(`${characterAppearance(entity)}-0-0`);
    const atlas=modern||scene.textures.exists('people')&&scene.textures.get('people').has(`${characterAppearance(entity)}-0-0`);
    let body;
    if(atlas)body=scene.add.image(0,0,modern?'people-v2':'people',`${characterAppearance(entity)}-0-0`).setOrigin(.5,82/96).setScale(.11);
    else {
...
function syncRasterCharacter(visual,entity) {
    ...
    const frame=visual.modern?(entity.motion?pose.frame:0):pose.frame?1+(pose.frame-1)%4:0;
    ...
    if(visual.atlas&&isRasterMap()){
        ...
        // Opaque body is approximately 66 pixels tall in the 96-pixel cell.
        visual.body.setScale(Math.min(36/min,64/zoom)/66);
    }
}
