"""2.6C.2: one reusable Eevee scene, ten checkpointed sheets; no network assets.
...
ROLES = {
    'player': ((.88,.80,.61),(.08,.16,.25),(.12,.065,.03),'jacket'),
...
def build():
    ...
    for direction in range(8):
        for pose in range(7):
            ...
            center=Vector(((direction-3.5)*1.6,0,0))+up*((3.5-pose)*2.4-2.05)
            ...
            phase=(pose-1)*math.tau/6 if pose else 0
            swing=.32*math.sin(phase) if pose else 0
            def part(name,loc,scale,mat):
                obj=bpy.data.objects.new(name,mesh.copy());bpy.context.collection.objects.link(obj)
                obj.parent=root;obj.location=loc;obj.scale=scale;obj.data.materials.clear();obj.data.materials.append(mats[mat]);return obj
            ...
            for side in [-1,1]:
                ...
                hip=(x,0,.83);knee=(x,stride*.45,.46+max(0,-stride)*.15)
                foot=(x,stride,.08+max(0,stride)*.15)
    ...
    scene.render.resolution_x=512;scene.render.resolution_y=672;scene.render.resolution_percentage=100
    ...
    camera.type='ORTHO';camera.ortho_scale=16.8;scene.camera=obj
