...
(async () => {
    ...
    const results = [];
    try {
        for (const scenario of ['missing-manifest', 'missing-categories', 'invalid-manifest', 'debug-history'].filter(s => !process.env.JEU_FALLBACK_SCENARIO || process.env.JEU_FALLBACK_SCENARIO === s)) {
            ...
            await page.route('**/*', async route => {
                ...
                if (scenario === 'missing-manifest' && name === 'assets/art-v2/manifest.json' || scenario === 'missing-categories' && name.startsWith('assets/art-v2/layers/') && name.endsWith('-night.png')) return route.fulfill({ status: 404, body: '' });
                let body;
                ...
                if (scenario === 'invalid-manifest' && name === 'assets/art-v2/manifest.json') {
            ...
            await fs.writeFile(path.join(root, `assets/art-v2/source/fallback-${scenario}.json`), JSON.stringify(results.at(-1), null, 2) + '\n');
            console.log('PASS 2.6B.2 fallback/debug/history', scenario);
            await context.close();
        }
        if (!process.env.JEU_FALLBACK_SCENARIO) await fs.writeFile(path.join(root, 'assets/art-v2/source/fallback-validation.json'), JSON.stringify(results, null, 2) + '\n');
